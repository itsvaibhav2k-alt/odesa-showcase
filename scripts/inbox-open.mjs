/**
 * Open the redesigned /inbox in a VISIBLE browser, already authenticated.
 *
 ...
 * headed browser straight on /inbox (seeded Galaxy org). The window stays open
 * until you Ctrl-C this process; the temp user is torn down on exit.
 *
 * Usage: BASE_URL=http://localhost:3100 node --env-file=.env.local scripts/inbox-open.mjs
 */
...
async function provisionGalaxy() {
  ...
  const email = `view.galaxy.${stamp}.${rand}@inbox.test`;
...
(async () => {
  ...
  const resp = await page.goto(`${BASE_URL}/inbox`, { waitUntil: 'networkidle', timeout: 30000 });
  console.log(`[inbox-open] opened ${page.url()} (HTTP ${resp?.status()}) as ${owner.email}`);
  console.log('[inbox-open] window is open — Ctrl-C here to close + clean up the temp user.');

  const cleanup = async () => {
    console.log('\n[inbox-open] tearing down…');
...
})().catch((e) => { console.error('[inbox-open] failed:', e.message); process.exit(1); });
